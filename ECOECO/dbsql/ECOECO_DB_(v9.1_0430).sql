...
-- 프로필 이미지 추가됨
CREATE TABLE `maker_prmaker_profileofile`
(
	`makerImgNo` INTEGER NOT NULL AUTO_INCREMENT COMMENT '프로젝트 썸네일 번호',
	`makerNo`    INTEGER NOT NULL COMMENT '프로젝트 번호',
	`makerImageFileName` VARCHAR(100) NOT NULL COMMENT '프로젝트 대표이미지 원본명',
	`makerImageSaveName` VARCHAR(100) NOT NULL COMMENT '프로젝트 대표이미지 저장명',
	`makerImagePath` VARCHAR(100) NOT NULL COMMENT '프로젝트 대표이미지 경로명',
	`projectNo`INTEGER NOT NULL COMMENT '프로젝트 번호',
	PRIMARY KEY ( `makerImgNo` )
) COMMENT = '메이커 프로필 이미지';
...
CREATE TABLE `project_thumbnail`( 
						    `projectImgNo` INTEGER NOT NULL AUTO_INCREMENT COMMENT '프로젝트 썸네일 번호',
							`projectNo`    INTEGER NOT NULL COMMENT '프로젝트 번호',
							`projectImageFileName`    VARCHAR(100) NOT NULL COMMENT '프로젝트 대표이미지 원본명',
							`projectImageSaveName`    VARCHAR(100) NOT NULL COMMENT '프로젝트 대표이미지 저장명',
							`projectImagePath` VARCHAR(100) NOT NULL COMMENT '프로젝트 대표이미지 경로명',
                            PRIMARY KEY ( `projectImgNo` )
)COMMENT = '프로젝트썸네일';

-- 환불 변경됨
CREATE TABLE `REFUND` (
  `refundNo` int NOT NULL AUTO_INCREMENT COMMENT '환불번호',
  `refundRequestDate` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '환불요청일',
  `refundDate` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '환불완료일',
...
CREATE TABLE `STORY` (
  ...
  `storyInfo` varchar(5000) NOT NULL COMMENT '스토리내용',
  `projectNo` int NOT NULL COMMENT '프로젝트 번호',
  PRIMARY KEY (`storyNo`)
) ENGINE=InnoDB COMMENT='스토리';

-- 스토리 대표 이미지 추가함
CREATE TABLE `STORY_REP_IMG`
(
    `storyImgNo` INTEGER NOT NULL AUTO_INCREMENT COMMENT '스토리 이미지 번호',
    `storyNo`    INTEGER NOT NULL COMMENT '스토리번호',
    `storyImageFileName`    VARCHAR(100) NOT NULL COMMENT '스토리 대표이미지 원본명',
    `storyImageSaveName`    VARCHAR(100) NOT NULL COMMENT '스토리 대표이미지 저장명',
    `storyImagePath`    VARCHAR(100) NOT NULL COMMENT '스토리 대표이미지 경로명',
 PRIMARY KEY ( `storyImgNo` )
) COMMENT = '스토리 대표 이미지';
